...
void Output::Quit() {
	...
	auto in = FileFinder::Save().OpenInputStream(OUTPUT_FILENAME, std::ios::ios_base::in);
	if (in) {
		...
		if (in.tellg() > log_size) {
			...
			auto out = FileFinder::Save().OpenOutputStream(OUTPUT_FILENAME, std::ios::ios_base::out);
			out.write(buf, read);
		}
	}
