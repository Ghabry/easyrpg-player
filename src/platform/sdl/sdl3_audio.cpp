...
void sdl_audio_callback(void* userdata, SDL_AudioStream* stream, int additional_amount, [[maybe_unused]] int total_amount) {
	...
	// FIXME MIGRATION: Audio crackles
	static_cast<GenericAudio*>(userdata)->Decode(buffer.data(), buffer.size());
	...
	GenericAudio(cfg)
{
	...
	audio_stream = SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_OUTPUT, &spec, sdl_audio_callback, this);
	...
	SetFormat(spec.freq, sdl_format_to_format(spec.format), spec.channels);
...
Sdl3Audio::~Sdl3Audio() {
	// FIXME MIGRATION Crash SDL_CloseAudioDevice(SDL_GetAudioStreamDevice(audio_stream));
}
