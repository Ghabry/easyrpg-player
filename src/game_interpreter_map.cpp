...
bool Game_Interpreter_Map::CommandCallEvent(RPG::EventCommand const& com) { // code 12330
	...
	switch (com.parameters[0]) {
		case 0: // Common Event
			...
			child_interpreter->Setup(Data::commonevents[evt_id - 1].event_commands, -1, 0, Data::commonevents[evt_id - 1].ID, -2);
			return true;
		case 1: // Map Event
			evt_id = com.parameters[1];
	...
	if (event != nullptr) {
		...
		child_interpreter->Setup(page.event_commands, event->GetId(), 0, event->GetX(), event->GetY());
	}
