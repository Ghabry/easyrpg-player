...
ZIPFilesystem::ZIPFilesystem(const FilesystemRef source_fs, const std::string& base_path, const std::string& encoding) :
	...
	std::string zip_encoding = encoding;
	if (FindCentralDirectory(zipfile, centralDirectoryOffset, centralDirectorySize, centralDirectoryEntries)) {
		if (zip_encoding.empty()) {
			...
			std::vector<std::string> encodings = ReaderUtil::DetectEncodings(filename_guess.str());
			for (auto enc : encodings) {
				std::string enc_test = ReaderUtil::Recode("\\", enc);
				if (enc_test.empty()) {
					// Bad encoding
		...
		while (ReadCentralDirectoryEntry(zipfile, filepath_arr, entry.fileoffset, entry.filesize)) {
			filepath = filepath_arr.data();
			filepath = ReaderUtil::Recode(filepath, zip_encoding);
			if ( Utils::StartsWith(filepath, inner_path)) {
				// remove the offset directory from the front of the path
				filepath = filepath.substr(inner_path.size(), filepath.size() - inner_path.size());
