...
public:
	...
	public:
		explicit vfs_istream(std::streambuf* sb, FilesystemRef fs, std::streamsize size);
...
public:
	 ...
	 */
	InputStream OpenInputStream(const std::string &name, std::ios_base::openmode m = (std::ios_base::openmode)0);
	 ...
	 * @return NULL if open failed.
	 */
	OutputStream OpenOutputStream(const std::string &name, std::ios_base::openmode m = (std::ios_base::openmode)0);
