...
*--renderer*::
  Renderer to use. The hardware renderer is only available when the Player was
  compiled with the SDL3 backend.
  Some games are unplayable or stutter without the hardware renderer.

  Possible options:
   - 'hardware'   - Hardware accelerated rendering (faster).
   - 'software'   - Software rendering. Use this when you encounter rendering
                    issues.
